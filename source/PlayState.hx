...
class PlayState extends MusicBeatState
{
	...
	public static var wigglesus:WiggleEffect;
	...
	public var alphys:Character;
	public var papyrus:Character;
	...
	public static var warning2:FlxSprite;
	...
	public var healthBarTwo:FlxBar;
	...
	override public function create()
	{
		...
		butterscotchPie = new FlxSprite(150, 300).loadGraphic(Paths.image('butterscotch_Pie'));
		butterscotchPie.updateHitbox();
		butterscotchPie.scrollFactor.set();

		instantNoodles = new FlxSprite(150, 335).loadGraphic(Paths.image('instant_Noodles'));
		instantNoodles.updateHitbox();
		instantNoodles.scrollFactor.set();

		legendaryHero = new FlxSprite(150, 360).loadGraphic(Paths.image('legendary_Hero'));
		...
		warning2 = new FlxSprite(-500, -310).loadGraphic(Paths.image('warningBlue'));
		warning2.updateHitbox();
		warning2.scale.x = 2.5;
		warning2.scale.y = 2.5;
		warning2.screenCenter(X);
		warning2.screenCenter(Y);
		...
		papyrus = new Character(0, 0, 'papyrus-chill');
		papyrus.alpha = 0;
		add(papyrus);
		add(dadGroup);
		add(boyfriendGroup);
		...
		alphys = new Character(0, 0, 'alphys');
		alphys.alpha = 0;
		add(alphys);
		...
		healthBarTwo = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
			'karma', 0, 2);	
		healthBarTwo.scrollFactor.set();
		// healthBarTwo(for karma)
		healthBarTwo.createFilledBar(0x00000000, FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]));
		healthBarTwo.visible = !ClientPrefs.hideHud;
		add(healthBarTwo);
		...
		txt = new FlxText(0, 300, FlxG.width,
			"Press 1 "
			+ "\nPress 2"
			+ "\nPress 3",
			32);		
		txt.setFormat("Mars Needs Cunnilingus", 32);
		...
		txt2 = new FlxText(200, 300, FlxG.width,
			bspNumber + "x\n"
			+ inNumber + "x\n"
			+ lhNumber + "x\n",
			32);		
		txt2.setFormat("Mars Needs Cunnilingus", 32);
		...
		scoreTxt.setFormat(Paths.font("Mars.Needs.Cunnilingus.ttf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	override public function update(elapsed:Float)
	{
		...
		if (hitbox.overlaps(boyfriend) && boyfriend.dodgetime < 12 && !cpuControlled)
		{
			health -= 100;
		}

		if (hitbox2.overlaps(boyfriend) && boyfriend.dodgetime > 12 && !cpuControlled)
		{
			health -= 100;
		}
		...
		if(hitbox.x == BF_X && boneBottaHit.animation.curAnim.finished)
		{
			boneBottaHit.alpha = 0;
		}

		if(hitbox2.x == BF_X && boneBottaHitBlue.animation.curAnim.finished)
		{
			boneBottaHitBlue.alpha = 0;
		}
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					if(daNote.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
						...
						else if(daNote.noteType == 'Alphys Note')
						{
							alphys.playAnim(animToPlay + altAnim, true);
							alphys.holdTimer = 0;
						}	
						else if(daNote.noteType == 'Papyrus Note')
						{
							papyrus.playAnim(animToPlay + altAnim, true);
							papyrus.holdTimer = 0;
						}						
						else {
	...
	function doDeathCheck() {
		if (health <= 0 && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("Got Dunked On - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Bone Attack':
				...
				warning.alpha = 1;
				FlxTween.tween(warning, {alpha: 0}, 0.4, {ease: FlxEase.quadInOut});
				...
				new FlxTimer().start(0.3, function(tmr:FlxTimer)
				{
					...
					FlxTween.tween(hitbox, {x: BF_X}, 0.4, {ease: FlxEase.quadInOut, startDelay: 0.5});
					new FlxTimer().start(1, function(tmr:FlxTimer)
					{
						FlxTween.tween(hitbox, {x: BF_X - 430}, 0.01, {ease: FlxEase.quadInOut});
					}, 0);
				}, 1);
			case 'Bone Attack Blue':

				FlxG.sound.play(Paths.sound('warning', 'shared'));
				warning2.alpha = 1;
				FlxTween.tween(warning2, {alpha: 0}, 0.4, {ease: FlxEase.quadInOut});
				...
				new FlxTimer().start(0.3, function(tmr:FlxTimer)
				{
					...
					FlxTween.tween(hitbox2, {x: BF_X}, 0.4, {ease: FlxEase.quadInOut, startDelay: 0.5});
					new FlxTimer().start(1, function(tmr:FlxTimer)
					{
						FlxTween.tween(hitbox2, {x: BF_X - 430}, 0.01, {ease: FlxEase.quadInOut});
					}, 0);
				}, 1);
		}
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement([ 'week0_nomiss_fdy', 'week0_nomiss_genocide', 'dissus', 'hype', 'two_keys', 'toastie', 'debugger']);
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if(storyWeek == 0 && storyDifficulty == 1 && !cpuControlled && !usedPractice)
					FlxG.save.data.aHumerusTrophy = true;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		if(!practiceMode && !cpuControlled) {
			...
			iconP1Tween = FlxTween.tween(iconP1.scale, {x: 1, y: 1}, 0.2, {
				onComplete: function(twn:FlxTween) {
					iconP1Tween = null;
				}
	...
	{
        healthBar.createFilledBar(FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]), 0xFFf23dc8);
	...
	function StrumPlayAnim(isDad:Bool, id:Int, time:Float) {
		...
		if(isDad) {
			...
			iconP2Tween = FlxTween.tween(iconP2.scale, {x: 1, y: 1}, 0.2, {
				onComplete: function(twn:FlxTween) {
					iconP2Tween = null;
				}
	...
	private function checkForAchievement(achievesToCheck:Array<String>):String {
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName)) {
				...
				{
					case 'week0_nomiss_fdy' | 'week0_nomiss_genocide' | 'dissus':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'FDY' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							var weekName:String = WeekData.getWeekFileName();
							switch(weekName) //I know this is a lot of duplicated code, but it's easier readable and you can add weeks with different names than the achievement tag
							{
								case 'week0':
									if(achievementName == 'week0_nomiss_fdy') unlock = true;
									
							}
						}
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'GENOCIDE' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
							{
								var weekName:String = WeekData.getWeekFileName();
								switch(weekName)
								{
									case 'week0':
										if(achievementName == 'week0_nomiss_genocide') unlock = true;
								}
							}
						if(isStoryMode && CoolUtil.difficultyString() == 'FDY' && !changedDifficulty && !usedPractice)
							{
								var weekName:String = WeekData.getWeekFileName();
								switch(weekName)
								{
									case 'week69':
										if(achievementName == 'dissus') unlock = true;
								}
							}
					case 'ur_bad':
