...
class TitleState extends MusicBeatState
{
	...
	var easterEggKeyCombination:Array<FlxKey> = [FlxKey.UP, FlxKey.DOWN, FlxKey.LEFT, FlxKey.LEFT, FlxKey.RIGHT]; //bb stands for bbpanzu cuz he wanted this lmao
	...
	{
		loadingImage = new FlxSprite(0,0).loadGraphic(Paths.image('loadingScreenNotDone'));
		loadingDone = new FlxSprite(0,0).loadGraphic(Paths.image('loadingScreenDone'));
	...
	function startIntro()
	{
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('titleBackground'));
		bg.screenCenter();
		...
		if(!FlxG.save.data.psykaEasterEgg || !easterEggEnabled) {
			gfDance = new FlxSprite(900, 325);
			gfDance.frames = Paths.getSparrowAtlas('distrustsans');
			gfDance.scale.x = 3;
			gfDance.scale.y = 3;
			gfDance.animation.addByPrefix('sansbump', '27_12_2021_17_14_17_REC_AdobeCreativeCloudExpress_', 14, true);
	...
	override function update(elapsed:Float)
	{
		...
		if(playstate)
		{
			MusicBeatState.switchState(new SITSState());
		}	
		if (FlxG.sound.music != null)
		...
		if (!transitioning && skippedIntro)
		{
			if(pressedEnter)
			{
				...
				{
					if (mustUpdate) {
						MusicBeatState.switchState(new InstructionsState());
					} else {
						MusicBeatState.switchState(new InstructionsState());
					}
					closedState = true;
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			{
				case 1:
					createCoolText(['The FNF Distrust team'], 45);
				...
				// credTextShit.screenCenter();
				case 5:
					createCoolText(['In Collabortation with'], 45);
				case 7:
					addMoreText('FNF', 45);
					addMoreText('Slaughter on a', 45);
					addMoreText('Friday Night', 45);
				// credTextShit.text += '\nNewgrounds';
				case 8:
					deleteCoolText();
					logoSpr.visible = false;
				...
				case 15:
					...
					addMoreText('Distrust'); // credTextShit.text += '\nFunkin';
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			remove(credGroup);
			FlxTween.tween(logoBl,{'scale.x': 0.65, 'scale.y': 0.65, y: -110, x: -135}, 2.4, {ease: FlxEase.expoInOut});
