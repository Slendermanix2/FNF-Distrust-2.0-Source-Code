...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var bgYellow:FlxSprite = new FlxSprite(0, 300).makeGraphic(FlxG.width, 386, 0xFFF9CF51);
		bgSprite = new FlxSprite(0, 300);
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			var weekThing:MenuItem = new MenuItem(0, bgSprite.y - 200, WeekData.weeksList[i]);
		...
		var tracksSprite:FlxSprite = new FlxSprite(FlxG.width * 0.07, bgSprite.y - 200).loadGraphic(Paths.image('Menu_Tracks'));
		tracksSprite.antialiasing = ClientPrefs.globalAntialiasing;
		add(tracksSprite);

		sansTrophy = new FlxSprite(-275, -500).loadGraphic(Paths.image('sansTrophy'));
		sansTrophy.alpha = 0;
		sansTrophy.scale.set(0.35, 0.35);
		sansTrophy.antialiasing = ClientPrefs.globalAntialiasing;
	...
	override function update(elapsed:Float)
	{
		...
		if(FlxG.save.data.aHumerusTrophy)
			sansTrophy.alpha = 1;
					...
					if (!weekIsLocked(curWeek))
						{
							...
							if (curWeek == 0 && !isCutscene) // Checks if the current week is Tutorial.
							{
								...
								{
									video.playMP4(Paths.video('Cutscene'), new LoadingState(new PlayState(), false, 'shared')); 
									isCutscene = true;
								});
