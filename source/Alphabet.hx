...
class Alphabet extends FlxSpriteGroup
{
	...
	public var isMenuItemTwo:Bool = false;
	...
	override function update(elapsed:Float)
	{
		...
		if (isMenuItemTwo)
		{
			var scaledY = FlxMath.remapToRange(targetY, 0, 1, 7, 7);

			var lerpVal:Float = CoolUtil.boundTo(elapsed * 9.6, 0, 1);
			y = FlxMath.lerp(x, (scaledY * yMult + 300) + (FlxG.height * 0.25) + yAdd, lerpVal);
			if(forceX != Math.NEGATIVE_INFINITY) {
				x = forceX;
				y = forceX;
			} else {
				x = FlxMath.lerp(x, (targetY * 600) + 90 + xAdd, lerpVal);
				y = FlxMath.lerp(x, (targetY * 0) + 90 + xAdd, lerpVal);
			}
		}
